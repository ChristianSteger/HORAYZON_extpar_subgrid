...
SUBROUTINE interpolate_fcor(horizon, swdir_cor, terrain_normal, &
  ...
  REAL(wp) :: dot_prod_s_h
  INTEGER :: idx_left, idx_right
  REAL(wp) :: angle_left, angle_right
  INTEGER:: idx_lin
  REAL(wp) :: deg2rad
  REAL(wp) :: weight_left
  REAL(wp), DIMENSION(3) :: sun_vec ! sun vector
  ...
  DO k = 1, 2
    ...
    ELSEIF (pos_norm >= 1.0_wp) THEN
      sun_vec = (/ COS(ztheta_sun)*SIN(deg2rad*azim_idx(k)*zihor), &
                   ...
                   SIN(ztheta_sun) /)
      dot_prod_s_h = MAX(sun_vec(3), 1e-5_wp)
      fcor(k) = (1.0_wp / dot_prod_s_h) * DOT_PRODUCT(sun_vec, terrain_normal)
    ...
    ELSE
      ...
      ELSEIF (idx_right == num_nodes) THEN
        ...
        dot_prod_s_h = MAX(sun_vec(3), 1e-5_wp)
        fcor_right = (1.0_wp / dot_prod_s_h) * DOT_PRODUCT(sun_vec, terrain_normal)
      ELSE
        idx_lin = (azim_idx(k) * (num_nodes - 2) + idx_left) - 1
        fcor_left = swdir_cor(idx_lin)
        idx_lin = (azim_idx(k) * (num_nodes - 2) + idx_right) - 1
  ...
  fcor_sun = (fcor(2)*(rad2deg*zphi_sun-zihor*ii) + fcor(1)*(zihor*(ii+1)-rad2deg*zphi_sun))/ zihor
